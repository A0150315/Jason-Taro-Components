...
export function getSystem() {
  ...
  if (/IOS/.test(ua)) {
    if (getEnv() === 'WEAPP') system = 'IOSWEAPP';else system = 'IOS';
...
export function getThisRouter() {
  ...
  if (getEnv() === 'WEAPP') {
    ...
    route = `${history[history.length - 1].route.replace('pages', '')}?${stringify(history[history.length - 1].options)}`;
  } else {
    route = `${window.location.pathname.replace('/pages', '')}${window.location.search}`;
  }
  return route;
}
